...
class AnimeWallpaper {
    ...
    scrapeFromWallpapersDotCom(search) {
        ...
        scrapeFromWallHaven(search) {
            if (!search) throw new error_1.default("Search parameters must be specified");
            
            // Handle default values for title, type, etc.
            if (!search.type) search.type = "sfw"; // Default to 'sfw'
            else if (!Object.keys(this.client.config.type).includes(search.type)) 
                throw new error_1.default("Please input one of 'sfw, sketchy, both'");
        
            // Use an empty string if title is not provided
            const queryTitle = search.title || '';
        
            return new Promise((resolve, reject) => {
                // Membuat request ke WallHaven dengan parameter tambahan untuk filter
                this.client.get.request(`${this.client.config.wallHaven}/search`, {
                    q: queryTitle, // Set to empty string if title is not provided
                    page: search.page,
                    purity: this.client.config.type[search.type],
                    ai_art_filter: !search.aiArt ? 1 : 0, // Adjust AI art filter
                    categories: 110,  // Mengatur kategori, misalnya 1 untuk wallpaper umum
                    purity: 100,
                    ratios: "portrait",  // Filter untuk gambar potret
                    sorting: "relevance",  // Sortir berdasarkan relevansi
                    order: "desc"  // Urutkan secara menurun
                })
                .then(x => {
                    const $ = cheerio_1.default.load(x.text);
                    const results = [];
                    $(".thumb-listing-page ul li .thumb").each((i, elm) => {
                        ...
                        // Ambil URL thumbnail
                        const thumbnailUrl = $(elm).find("img").attr("data-src") || $(elm).find("img").attr("src");
        
                        // Ambil URL gambar full
